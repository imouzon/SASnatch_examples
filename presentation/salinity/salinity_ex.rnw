...
<<chunk1, echo=FALSE, cache=TRUE, comment=NA, include = TRUE>>=
...
<<SASplots-GROSS, echo=FALSE, comment=NA, cache=TRUE, include = TRUE>>=
...
<<SASmixed, SASnatch = c('salinity',''), eval=FALSE, echo=FALSE, cache=FALSE, include = TRUE>>=
   proc mixed method = type3 data = salinity; 
     class trt container; 
     model weight = trt; 
...
%-- WHAT A CHEAT! THIS ISNT HOW WE DO!
<<SASmixedcode, echo=FALSE, comment=NA, include = TRUE>>=
   catSnatch(SASmixed.snatch)
@
